...
@if(count($orders) > 0)
    ...
    @foreach($orders as $order)
        <div class="">
            <div class="col-lg-12 col-md-12 col-sm-12 ordered">场次：{{$order->detail}}
                <a class="seat_no">座号：{{$order->description}}</a>
                <a class="refund pull-right" href="/scenes/create" role="button">退票</a>
            </div>
                    ...
                    @foreach($seats as $seat)
                        @if($seat->line == $i)
                            ...
                            {{--<label class="btn btn-primary">--}}
                                <img id="'seat_'+ {{$i}} +'_' + {{$seat->row}}" @if($seat->available == 1) src="/photos/seat.png" class="seat img-thumbnail img-check" @else src="/photos/seat_unavailable.png" class="seat" @endif alt="..." onclick="buyTicket({{$seat}})">
...
//        alert(tables);
        for (var i = 0; i < tables.length; i++) {  // 逐个改变
            tables[i].style.width = '{{100/$rows -4}}%';  // 宽度改为15%
